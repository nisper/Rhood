...
function Menu({ align = "left", className, children, ...props }: MenuProps) {
  return (
    <div className={cn("flex w-max flex-col rounded-xl bg-[var(--parser-fill-contrast)] p-1 shadow-[0_3px_14px_rgba(0,0,0,0.25)] [&>*]:w-full", align === "left" ? "self-start" : "self-end", className)} {...props}>
      {children}
    </div>
  )
}
