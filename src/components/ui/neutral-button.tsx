...
type NeutralButtonSize = "lg" | "md" | "sm" | "xsm"
type NeutralButtonVariant = "contained" | "outlined" | "text"

type NeutralButtonProps = React.ComponentProps<"button"> & {
  counter?: boolean
...
const sizeClasses: Record<NeutralButtonSize, string> = {
...
const textSizeClasses: Record<NeutralButtonSize, string> = {
  lg: "px-2 py-4",
  md: "px-2 py-2",
  sm: "px-2 py-[5px]",
  xsm: "px-2 py-1",
...
const iconOnlyClasses: Record<NeutralButtonSize, string> = {
  lg: "size-14 p-0",
  md: "h-10 w-11 p-0",
  sm: "size-[30px] p-0",
  xsm: "size-6 p-0",
...
const iconSlotClasses: Record<NeutralButtonSize, string> = {
  lg: "w-6",
  md: "w-6",
  sm: "w-4",
  xsm: "w-4",
...
const counterClasses: Record<NeutralButtonSize, string> = {
...
const variantClasses: Record<NeutralButtonVariant, string> = {
  contained:
    "border border-transparent bg-[var(--parser-fill-neutral)] text-[var(--parser-text-neutral-primary)] hover:bg-[var(--parser-fill-neutral-hover)]",
  outlined:
    "border border-[var(--parser-border-neutral)] bg-transparent text-[var(--parser-text-neutral-primary)] hover:bg-[var(--parser-fill-neutral-hover)]",
  text: "border border-transparent bg-transparent text-[var(--parser-text-neutral-primary)] hover:bg-[var(--parser-fill-neutral-hover)]",
}

const disabledClasses: Record<NeutralButtonVariant, string> = {
  contained:
...
/**
 * Parser neutral button matching the Figma `button-neutral` component set.
 */
function NeutralButton({
  ...
  return (
    <button
      className={cn(
        "inline-flex shrink-0 items-center justify-center overflow-hidden rounded-lg font-semibold transition-colors duration-150 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--parser-focus-ring)] disabled:pointer-events-none",
        sizeClasses[size],
...
export type { NeutralButtonProps, NeutralButtonSize, NeutralButtonVariant }
