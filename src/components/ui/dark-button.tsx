...
type DarkButtonSize = "lg" | "md" | "sm" | "xsm"
type DarkButtonVariant = "contained" | "outline" | "text"

type DarkButtonProps = React.ComponentProps<"button"> & {
  counter?: boolean
...
const sizeClasses: Record<DarkButtonSize, string> = {
...
const textSizeClasses: Record<DarkButtonSize, string> = {
  lg: "px-2 py-4",
  md: "px-2 py-2",
  sm: "px-2 py-[5px]",
  xsm: "px-2 py-1",
...
const iconOnlyClasses: Record<DarkButtonSize, string> = {
  lg: "size-14 p-0",
  md: "h-10 w-11 p-0",
  sm: "size-[30px] p-0",
  xsm: "size-6 p-0",
...
const iconSlotClasses: Record<DarkButtonSize, string> = {
  lg: "w-6",
  md: "w-6",
  sm: "w-4",
  xsm: "w-4",
...
const counterClasses: Record<DarkButtonSize, string> = {
...
const variantClasses: Record<DarkButtonVariant, string> = {
  contained:
    "border border-transparent bg-[var(--parser-fill-neutral-dark)] text-[var(--parser-text-primary-contrast)] hover:bg-[var(--parser-fill-neutral-dark-hover)]",
  outline:
    "border border-[var(--parser-border-neutral-dark)] bg-transparent text-[var(--parser-text-primary-static)] hover:bg-[var(--parser-fill-neutral-hover)]",
  text: "border border-transparent bg-transparent text-[var(--parser-text-primary-static)] hover:bg-[var(--parser-fill-neutral-hover)]",
}

const disabledClasses: Record<DarkButtonVariant, string> = {
  contained:
...
/**
 * Parser dark button matching the Figma `button-dark` component set.
 */
function DarkButton({
  children = "Label",
  className,
  ...
  const counterTone =
    variant === "contained" && !disabled
      ? "bg-[var(--parser-fill-contrast-static)] text-[var(--parser-text-primary-static)]"
...
export { DarkButton }
export type { DarkButtonProps, DarkButtonSize, DarkButtonVariant }
