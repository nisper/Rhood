...
type ErrorButtonSize = "lg" | "md" | "sm" | "xsm"
type ErrorButtonVariant = "contained" | "outlined" | "text"

type ErrorButtonProps = React.ComponentProps<"button"> & {
  counter?: boolean
...
const sizeClasses: Record<ErrorButtonSize, string> = {
...
const textSizeClasses: Record<ErrorButtonSize, string> = {
  lg: "px-2 py-4",
  md: "px-2 py-2",
  sm: "px-2 py-[5px]",
  xsm: "px-2 py-1",
...
const iconOnlyClasses: Record<ErrorButtonSize, string> = {
  lg: "size-14 p-0",
  md: "h-10 w-11 p-0",
  sm: "size-[30px] p-0",
  xsm: "size-6 p-0",
...
const iconSlotClasses: Record<ErrorButtonSize, string> = {
  lg: "w-6",
  md: "w-6",
  sm: "w-4",
  xsm: "w-4",
...
const counterClasses: Record<ErrorButtonSize, string> = {
...
const variantClasses: Record<ErrorButtonVariant, string> = {
  contained:
    "border border-transparent bg-[var(--parser-fill-error)] text-[var(--parser-text-primary-contrast)] hover:bg-[var(--parser-fill-error-hover)]",
  outlined:
    "border border-[var(--parser-border-error-light)] bg-transparent text-[var(--parser-text-error)] hover:bg-[var(--parser-fill-error-light-hover)]",
  text: "border border-transparent bg-transparent text-[var(--parser-text-error)] hover:bg-[var(--parser-fill-error-light-hover)]",
}

const disabledClasses: Record<ErrorButtonVariant, string> = {
  contained:
...
/**
 * Parser error button matching the Figma `button-error` component set.
 */
function ErrorButton({
  ...
  const counterTone =
    variant === "contained" && !disabled
      ...
      : "bg-[var(--parser-fill-error)] text-[var(--parser-text-primary-contrast)]"
...
export { ErrorButton }
export type { ErrorButtonProps, ErrorButtonSize, ErrorButtonVariant }
