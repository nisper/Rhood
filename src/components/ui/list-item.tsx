...
function ListItem({
  ...
  const isHighlighted =
    !disabled && !isSelected && (state === "hovered" || state === "focused");
  ...
  return (
    <div
      ...
      className={cn(
        ...
        hasPaddingX && "px-[calc(var(--spacing)*2)]",
    ...
    >
      {startIcon && (
        <span
          className={cn(
            "mr-[calc(var(--spacing)*4)] flex size-[calc(var(--spacing)*6)] shrink-0 items-center justify-center",
            disabledOpacity,
          )}
        >
          <Star
            aria-hidden="true"
            className="size-[calc(var(--spacing)*6)]"
            strokeWidth={2}
          />
        </span>
      )}
      <span
        className={cn(
          "flex min-w-0 flex-1 flex-col py-[calc(var(--spacing)*1)]",
          disabledOpacity,
        )}
      >
        <span
      ...
      {iconButton && (
        <Button
          aria-label="Действие со строкой"
          appearance="ghost"
          disabled={disabled}
          iconOnly
          size="sm"
