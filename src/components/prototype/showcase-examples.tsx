...
export function ShowcaseExamples() {
  return <div className="grid min-w-0 gap-10">
    <Section description="ShowcaseSurface объединяет примеры на общей поверхности. Панели добавляются через children — их может быть столько, сколько нужно." settings={["direction: horizontal", "ShowcasePanel.tone: white · transparent"]} title="Горизонтальная композиция">
    ...
    <Section description="direction=vertical размещает панели друг под другом. Прозрачная панель показывает фон родительской поверхности." settings={["direction: vertical", "ShowcasePanel.tone: white · transparent"]} title="Вертикальная композиция">
