      ...
      <section className="grid gap-4">
        <div>
          <div>
            ...
            <p className="text-sm leading-5 text-[color:var(--rhood-theme-text-neutral-secondary)]">{tokens.length} токенов в выбранной коллекции</p>
          </div>
        </div>
        ...
        <Table className="min-w-[940px] border border-[color:var(--rhood-theme-border-light)] bg-[var(--rhood-theme-surface-bg)]">
          ...
          {tokens.map((token) => <div className="flex border-b border-[color:var(--rhood-theme-border-light)] last:border-b-0" key={`${token.collection}/${token.name}`} role="row">
            ...
            <TableCell custom role="body" type="text" width={200}><code className="font-mono text-sm leading-5 text-[color:var(--rhood-theme-text-neutral-primary)]">{token.name}</code></TableCell>
            <TableCell custom role="body" type="text" width={180}><code className="font-mono text-sm leading-5 text-[color:var(--rhood-theme-text-neutral-primary)]">{token.resolvedValue ?? "—"}</code></TableCell>
